...
def create_true_color_composite(qkm_file, hkm_file, output_filename):
    ...
    try:
        ...
        print("Scaling and creating true colour composite...")
...
def create_false_colour_composite(qkm_file, hkm_file, output_filename):
    """
    Create a false-color composite image using bands from QKM and HKM MODIS files.
    
    This function combines 250m (blue band) and 500m (SWIR2 and SWIR3 bands) MODIS data
    ...
    """
    try:
        print("\nCreating false-color composite image...")
        ...
        composite_ds = driver.Create(
            ...
            target_width, 
            target_height, 
            4,  # 4 bands: SWIR2, SWIR3, Blue, and Alpha
            gdal.GDT_Byte,
            options=['ALPHA=YES', 'COMPRESS=LZW']  # Add alpha channel and compression
        )
        ...
        if qkm_band_count >= 1:
            blue_array = qkm_ds.GetRasterBand(1).ReadAsArray()  # Band 3 (blue)
        else:
            print("QKM file doesn't have enough bands")
            return None
        ...
        if hkm_band_count >= 3:
            swir2_array = hkm_ds.GetRasterBand(2).ReadAsArray()  # Band 6 (SWIR2)
            swir3_array = hkm_ds.GetRasterBand(3).ReadAsArray()  # Band 7 (SWIR3)
        else:
            print("HKM file doesn't have enough bands")
            return None
        ...
        # Scale values with more robust handling
        print("Scaling and creating false colour composite...")
        ...
        # Scale each band
        swir2_scaled = scale_array(swir2_array)
        swir3_scaled = scale_array(swir3_array)
        blue_scaled = scale_array(blue_array)
        
        # Create alpha channel based on valid data
        alpha_channel = np.ones_like(blue_scaled) * 255  
        
        valid_data_mask = (swir2_array > 0) & (swir3_array > 0) & (blue_array > 0)
        valid_data_mask = valid_data_mask & ~np.isnan(swir2_array) & ~np.isnan(swir3_array) & ~np.isnan(blue_array)
        alpha_channel[~valid_data_mask] = 0
        
        # Write bands to composite
        composite_ds.GetRasterBand(1).WriteArray(swir2_scaled)
        composite_ds.GetRasterBand(2).WriteArray(swir3_scaled)
        composite_ds.GetRasterBand(3).WriteArray(blue_scaled)
        composite_ds.GetRasterBand(4).WriteArray(alpha_channel)  # Alpha channel
        ...
        print(f"Error creating false-color composite: {str(e)}")
        import traceback
        traceback.print_exc()
        return None
...
if __name__ == "__main__":
    ...
    if matching_pairs:
        print(f"\nFound {len(matching_pairs)} matching QKM/HKM granule pairs for composites")
        ...
        for i, (qkm_idx, hkm_idx) in enumerate(matching_pairs):
            ...
            composite_tc_output = f"prince_of_wales_truecolor_{i+1}.tiff"
            create_true_color_composite(qkm_hdf_file, hkm_hdf_file, composite_tc_output)
            
            #create false colour composite
            composite_fc_output = f"prince_of_wales_falsecolor_{i+1}.tiff"
            create_false_colour_composite(qkm_hdf_file, hkm_hdf_file, composite_fc_output)
